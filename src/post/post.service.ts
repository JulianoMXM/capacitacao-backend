...
export class PostService {
  ...
  async create(createPostDto: CreatePostDto, id: string) {
    const baseSlug = slugify(createPostDto.title, { lower: true, strict: true })
    ...
    while(!isUnique){
      ...
      if(attempts > 10){
        throw new InternalServerErrorException('Não foi possível gerar uma slug única')
      }
    }
  ...
  async findOneFromUser(userId: string, slug: string) {
    ...
    if(post.author.id !== userId){
      throw new ForbiddenException('Você não tem permissão para ver este post.')
    }
  ...
  async update(userId: string, postId: string, updatePostDto: UpdatePostDto) {
    ...
    if(updatePostDto.title){
      const baseSlug = slugify(updatePostDto.title, { lower: true, strict: true })
